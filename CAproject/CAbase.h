...
public:
  ...
  int CellEvolutionDirection( int x, int y );
  int CellEvolutionMove( int x, int y, int l);
  ...
  void WorldEvolutionLifePredator( int lifeTime );
...
//
// The cell becomes alive if at least one of its neighbours is alive.
...
//
// Limited growth case with Moore's neighbourhood(northWestCell, northCell, northEastCell, westCell, eastCell, southWestCell, southCell, southEastCell).
//
// The cell becomes alive if ONLY one of its neighbours is alive.
// The cell, once alive, remains alive forever.
//
...
//
// Limited growth case with von Neumann neighbourhood(northCell, westCell, eastCell, southCell).
//
// The cell becomes alive if ONLY one of its neighbours is alive.
// The cell, once alive, remains alive forever.
//
...
//
// Predator-Victim game based on CA.
//
// There are three types of cells:
//
// Food cell with state 11.  Non-moving. The lifetime is infinite
// Victim cell with state 10.  Random Moving, has the lifetime
// Predator cell with state 9. Random Moving, has the lifetime
...
//
inline int CAbase::CellEvolutionDirection( int x, int y)  //Set direction of move for current alive cell
{
    std::vector<int> vec;
    bool victim = false;
    bool food = false;

    int dir[3][3] = {0,4,0,                         // Array for faster checking of cell direction
                     2,3,8,
                     0,6,0};

    if(IsAlive(x,y) == 9 || IsAlive(x,y) == 10){ // check the cell is alive or not
        for(int i = -1; i <= 1; i++){
            for(int j = -1; j <= 1; j++){
                if(abs(i) == abs(j)) continue;

                if( IsAlive(x+i,y+j)!= -1 && IsAlive(x+i,y+j)!= 9 && IsAlive(x+i,y+j)!= 10 && IsAlive(x+i,y+j)!=11 && !victim && !food )
                    vec.push_back(dir[i+1][j+1]);    // set direction

                if( IsAlive(x,y) == 9 && IsAlive(x+i, y+j) == 10 ){ //check if there is a predator in the current cell and a victim is in the neighbouring cell
                    if(!victim){
                        vec.clear();
                        victim = true;
                    }
                ...
                if( IsAlive(x,y) == 10 && IsAlive(x+i, y+j) == 11 ){ //check if there is a victim in the current cell and a food is in the neighbouring cell
                    if(!food){
                        vec.clear();
                        food = true;
                    }
                ...
                if( IsAlive(x,y) == 10 && IsAlive(x+i, y+j) == 9 ){
                    vec.clear();
                    return 1;
                }
            }
        ...
        if(vec.size() == 0){
            SetColor(x, y, 3);  // set the cell doesn't move
            return 1;
        }

        int RandomMove = rand()%int(vec.size());
        SetColor(x, y, vec[RandomMove]); // set the cell move on random direction
...
inline int CAbase::CellEvolutionMove( int x, int y, int l)  //Universe evolution for current cell (move or stand, live or die, etc)
{
    std::vector<int> vec;
    bool predator = false;

    int dir[3][3] = {0,6,0,                         // Inverse array for faster checking of cell direction
                     8,3,2,
                     0,4,0};

    if(IsAlive(x,y) == 9 && GetLife(x,y) != 0){
        SetAliveEvo(x,y,9);
        SetLifeEvo(x,y, GetLife(x,y) - 1);    //set the lifetime of cell to one less
        return 1;
    }
    ...
    for(int i = -1; i <= 1; i++){
        for(int j = -1; j <= 1; j++){
            ...
            if(GetColor(x+i,y+j) == dir[i+1][j+1]){  //check that the neighbouring cell has direction on current cell

                if(IsAlive(x+i,y+j) != 9 && !predator)
                    vec.push_back(dir[i+1][j+1]);  // add the neighbouring cell

                if(IsAlive(x+i,y+j) == 9 ){
                    if(!predator){
                        vec.clear();
                        predator = true;
                    }
    ...
    if(vec.size() == 0 && IsAlive(x,y) == 10 && GetLife(x,y) != 0){
        SetAliveEvo(x, y, 10);
        SetLifeEvo(x,y, GetLife(x,y) - 1);  //set the lifetime of cell to one less
        return 1;
    }

    if(vec.size() == 0 && IsAlive(x,y) == 11){
        SetAliveEvo(x,y,11);
        return 1;
    }
    ...
    int RandomMove = rand()%int(vec.size());  //random selection of a neighbour

    switch (vec[RandomMove]) {  //delete information about the selected neighbour
    case 2:
        SetLife(x,y, GetLife(x,y+1));
        SetAlive(x,y+1,0);
        SetColor(x,y+1,0);
        SetLife(x,y+1,0);
        SetAliveEvo(x,y+1,0);
    ...
    if(GetLife(x,y) != 0){
        if(predator){
            SetAliveEvo(x, y, 9);  //move the selected neighbour
            if(IsAlive(x,y) == 10)
                SetLifeEvo(x,y, l);     //set the lifetime of cell to full value
            else
                SetLifeEvo(x,y, GetLife(x,y) - 1);      //set the lifetime of cell to one less
        }
        else{
            SetAliveEvo(x, y, 10);  //move the selected neighbour
            if(IsAlive(x,y) == 11)
                SetLifeEvo(x,y, l);         //set the lifetime of cell to full value
            else
                SetLifeEvo(x,y, GetLife(x,y) - 1);      //set the lifetime of cell to one less
...
//
// Run evolution for every cell of universe array in "Predator-Victim" mode
// After full evolution, new state of of universe have to be copied into current state
//

inline void CAbase::WorldEvolutionLifePredator( int lifeTime )
{
    for( int ix = 1; ix <= Nx; ix++ ) {             //Set direction of move for every alive cell
      for( int iy = 1; iy <= Ny; iy++ ) {
    ...
    for( int ix = 1; ix <= Nx; ix++ ) {             //Universe evolution for every cell
      for( int iy = 1; iy <= Ny; iy++ ) {
          CellEvolutionMove( ix, iy, lifeTime );
      }
    }
    ...
    for( int ix = 1; ix <= Nx; ix++ ) {         //Copy new state to current universe
      for( int iy = 1; iy <= Ny; iy++ ) {
        world[iy*(Nx+2)+ix] = worldNew[iy*(Nx+2)+ix];
        worldColor[iy*(Nx+2)+ix] = 0;
        worldLife[iy*(Nx+2)+ix] = worldLifeNew[iy*(Nx+2)+ix];
      }
    }
}
...
// Diffusion of two gases with Margolus neighbourhood
// Margolus neighbourhood in 2-dimensional case - 2x2 square block
// CA divides universe on such blocks and change every block independently
// CA works in 2 streps: 1.Margolus neighbourhood starts from (X, Y); 2.Starts from (X+1, Y+1)
// 1st gas - "0", 2nd gas - "1"
// Evolution: cell values inside of every Margolus neighbourhood shifts clockwise or counterclockwise
// Direction of shift is chosen randomly for every algorithm step end every Margolus neighbourhood
...
// World evolution function for calculations with Margolus neighbourhood
